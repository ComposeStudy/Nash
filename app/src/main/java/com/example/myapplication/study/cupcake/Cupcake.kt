...
@Composable
fun ButtomButton(route: String, navController: NavController) {
    Button(onClick = {
        when (route) {
            ...
            CupcakeRoute.Summary.name -> navController.popBackStack(route = CupcakeRoute.Start.name, inclusive = true)
        }
    }) {
        when (route) {
...
//    val state = cupcakeViewModel.cupcakeState.collectAsState()
    ...
    Column {
        ...
        ButtomButton(route = route, navController = navController)
