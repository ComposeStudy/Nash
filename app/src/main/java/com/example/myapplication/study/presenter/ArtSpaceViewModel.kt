package com.example.myapplication.study.presenter
...
//    private val data = flowOf(arrArt)
    private var selectIndex = 0
    private val _artIndexState = MutableStateFlow(arrArt[0])
    val uiState = _artIndexState.map {
        Log.e("smaohao" , "_artIndexState.map state $it")
//        delay(3000)
        ChangeArt(it)
    }.stateIn(
    ...
    fun eventHandler(left: Boolean) {
        Log.e("smaohao" , "eventHandler left $left")
        viewModelScope.launch {
            ...
            _artIndexState.emit(arrArt[selectIndex])
        }
    }
}
...
//    data class Init(val artData: Art) : ArtSpaceUiState
